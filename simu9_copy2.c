...
#define M_END       (2.0e-2)	//熱交換流体の質量の上限値
...
void init_layer(double rho_r[LAYER], int layer_parameter[LAYER+1], int height)
{
  /* GdNi2密度:9.73 */
  /* DyAl2密度:6.10 */
  /* GdAl2密度:5.71 */
	/* HoAl2密度:6.09 */
	/* ErCo2密度:10.47 */
	rho_r[0] = 10.47e3;	//1層目の磁性体の密度
	rho_r[1] = 6.09e3;	//2層目の磁性体の密度
	layer_parameter[0] = 0;		//高温端の位置
	layer_parameter[1] = height;	//層の区切りの位置
...
//Temp[K]	density[kg/m3]	Cp[J/kg-K]	viscosity[kg/m-s]	k[W/m-K]=[J/m-K-s]
	double gas_data[5];
  int height;
  int height_interval = 40;

  for(height = 200; height >= 0; height -= height_interval){
    char s1[N] = ("0-");
    char s2[N] = ("\0");
    char s3[N] = ("-200_siu9-out_copy2.txt");
    char *s4;
    char *s5;
    sprintf(s2, "%d", height);
    s4 = strcat(s1, s2);
    s5 = strcat(s4, s3);
  	...
  	fp_simu9_auto   = fopen(s5,   "w");
  	fp_simu9_data   = fopen("simu9-data.txt",  "w");

  	/* ファイルの読み込み */
  	fp_simu9_He5atm = fopen("He5atm-data.txt", "r");
  	fp_capa[0]      = fopen("(ErDy)Co2_kg/ErCo2_C_kg.txt", "r"); /* ※ */
  	fp_capa[1]      = fopen("LaAl2_kg_HoAl2_select_0.05-6T/C_Tc=30K_Ho3+_0.05-6T_LaAl2.txt", "r");	/* ※ */
  	fp_entropy[0] = fopen("(ErDy)Co2_kg/ErCo2_S_kg.txt", "r");	/* ※ */
  	fp_entropy[1] = fopen("LaAl2_kg_HoAl2_select_0.05-6T/S_Tc=30K_Ho3+_0.05-6T_LaAl2.txt", "r");	/* ※ */
  ...
  */
  ...
  //	rho_r[0]  = RHO_MAG1;        //[kg/m3]:magnetic density
  	init_layer(rho_r, layer_parameter, height);
  	eps       = 0.36;           //AMRベッドの空孔率
