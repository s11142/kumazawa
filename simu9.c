...
#define NUM            (12000) //1000
#define CYCLE_END      (1000) //1000
...
#define DM          (5.0e-3)	//熱交換流体の質量の変化幅
#endif
/*
#define T_HIGH         (77.0)    //high end[K]
#define T_LOW          (60.0)    //low  end[K]
*/
...
void init_layer(double rho_r[LAYER], int layer_parameter[LAYER+1])
{
	rho_r[0] = 9.73e3;	//1層目の磁性体の密度
	rho_r[1] = 9.73e3;	//2層目の磁性体の密度
	layer_parameter[0] = 0;		//高温端の位置
	layer_parameter[1] = 25;	//層の区切りの位置
	layer_parameter[2] = 200;	//低温端の位置
}


//== prototype declaration ===============================================
void initialize(double Tgas[SPACE][TIME], double Treg[SPACE][TIME], double, double);
...
void output(FILE *fp_Tgas, FILE *fp_Treg, FILE *fp_simu9_auto, FILE *fp_simu9_data,
						...
						double h, double rho_r[LAYER], double gas_data[5], double t_high, double t_low);
...
//Temp[K]	density[kg/m3]	Cp[J/kg-K]	viscosity[kg/m-s]	k[W/m-K]=[J/m-K-s]
	...
	int t_interval = 5; //※
	int t_low_max = 65; // ※
	int t_high = (int)T_HIGH;
	int t_low;

	for(t_low = (int)T_LOW; t_low <= t_low_max; t_low += t_interval){
		char s1[N] = ("\0");
		char s2[N] = ("\0");
		char s3[N] = ("_T_high = ");
		char s4[N] = ("T_low = ");
		char s5[N] = ("K_simu9-out_copy.txt");
		char *s6;
		char *s7;
		char *s8;
		char *s9;
		...
		fp_Tgas         = fopen("Tgas9_copy.txt",       "w");
		fp_Treg         = fopen("Treg9_copy.txt",       "w");
		fp_simu9_auto   = fopen(s9,   "w");
		fp_simu9_data   = fopen("simu9-data_copy.txt",  "w");

		/* ファイルの読み込み */
		fp_simu9_He5atm = fopen("He5atm-data.txt", "r");
		fp_capa[0]      = fopen("Gd3+_LaNi2_kg_GdNi2_0-6T_select/C_Tc=70K_Gd3+_0-6T.txt", "r"); /* ※ */
		fp_capa[1]      = fopen("Gd3+_LaNi2_kg_GdNi2_0-6T_select/C_Tc=60K_Gd3+_0-6T.txt", "r");	/* ※ */
		fp_entropy[0] 	= fopen("Gd3+_LaNi2_kg_GdNi2_0-6T_select/S_Tc=70K_Gd3+_0-6T.txt", "r");	/* ※ */
		fp_entropy[1] 	= fopen("Gd3+_LaNi2_kg_GdNi2_0-6T_select/S_Tc=60K_Gd3+_0-6T.txt", "r");	/* ※ */
	...
	*/
	...
	/*ファイルが用意されているかどうかの確認 */
		...
		fprintf(fp_Tgas, "TimeStep coldend hotend\n");
		fprintf(fp_Treg, "SpaceStep hot-to-cold mag cold-to-hot demag\n");
	...
	//		h = 3000.0;
			...
			initialize(Tgas, Treg, (double)t_high, (double)t_low);
	...
	//			printf("cycle = %d\n", cycle);
			...
			output(fp_Tgas, fp_Treg, fp_simu9_auto, fp_simu9_data,
							...
							dt, C_gas, Ntu, lambda,
							Treg, Tgas, h, rho_r, gas_data, (double)t_high, (double)t_low);
...
void initialize(double Tgas[SPACE][TIME], double Treg[SPACE][TIME], double t_high, double t_low)
{
	int i, j;
	double deltaTemp;
	double Thigh = t_high;
...
*/
	...
	for(i = 0; i < SPACE; i++){
		fprintf(fp_Treg, "%d %f %f %f %f\n", i,
											Treg[i][n4], Treg[i][n2],
											Treg[i][n2+n4], Treg[i][TIME-1]);
	}
	#endif
